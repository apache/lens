...
public class GrillStatement {
  ...
  public List<QueryHandle> getAllQueries(String status, String user) {
    WebTarget target = getQueryWebTarget(ClientBuilder
        .newBuilder().register(MultiPartFeature.class).build());
    List<QueryHandle> handles = target.queryParam("sessionid", connection.getSessionHandle()).queryParam("state", status).queryParam("user", user).request().get(
