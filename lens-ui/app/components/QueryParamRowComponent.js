...
class QueryParamRow extends React.Component {
  ...
  render () {
    ...
    let collectionTypeBox = dataType === 'DATE_TIME' ?
      (<select className='form-control' required value='SINGLE'
        onChange={this._handleChange('ChangeCollectionType')}>
      ...
      switch (elementType) {
        ...
        case 'ChangeDatePicker':
          if (typeof arg === 'object') {
            paramChange = _.assign({}, state, {
              defaultValue: arg.format('YYYY-MM-DD HH:mm:ss')
            });
          }
          break;
        ...
        case 'ChangeDataType':
          ...
          paramChange = _.assign({}, state, {
            ...
            defaultValue: val,
            // make `collectionType` SINGLE when `dataType` is DATE_TIME
            collectionType: arg.target.value === 'DATE_TIME' ? 'SINGLE' : state.collectionType
          });
          break;
  ...
  getDefaultValueInput (collectionType, dataType) {
    ...
    if (collectionType === 'SINGLE') {
      if (dataType === 'DATE_TIME') {
        valueBox = <DateTime defaultValue={this.state.paramChange.defaultValue}
          onChange={this._handleChange('ChangeDatePicker')} dateFormat='YYYY-MM-DD' className='datePicker'
          timeFormat='HH:mm:ss'/>;
      } else {
